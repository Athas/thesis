...
restrictions of our handling of loop fusion.
...
across \texttt{zip}/\texttt{unzip}, and more generally the case where
the output of a producer is used by several consumers.  The algorithm
presented in this chapter is capable of fusing such cases whenever
possible without duplicating computation, as demonstrated on
\cref{fig:fusion-multiple-consumers}.

\begin{figure}
\begin{subfigure}[bt]{0.5\textwidth}
\centering
\begin{bcolorcode}
let \emp{b} = map(op+(1), a) in
let c = map(op+(2), \emp{b}) in
let d = map(op+(3), \emp{b}) in
map(op+, zip(c,d))
\end{bcolorcode}
\caption{Unfused}
\end{subfigure}%
