...
expressions whose result is eventually consumed.
...
In order to keep the exposition simpler, I will ignore \texttt{loop}
and \texttt{let-with} (except with respect to upholding uniqueness
constraints), and only talk about hoisting normal
\texttt{let}-bindings.
...
is a more efficient strategy. If we make the rebinder perform the CSE
optimisation when we insert non-hoistable bindings into the syntax
tree, we only have to perform a single program traversal.  I will try
to provide an intuition for the approach using the above example:
