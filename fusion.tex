...
exactly once in every relevant consumer.  This property can be
guaranteed through trivial rewriting prior to performing function
composition, as illustrated on \fref{fig:single-input-transform}.
This guarantees that each output of a producer is bound to exactly one
parameter of a consumer's function, making it easier to describe the
relationship between producer and consumer.\footnote{In the actual
  ...
  composition uses more complicated bookkeeping, but presenting the
  full method details would obscure the exposition of the central
  mechanism.}
