...
\cref{chap:internal}.  We will also assume that all instances of
...
The \textit{fusion algebra} in \cref{sec:fusionalgebra} describes
superficially which producer-consumer pairs can be fused, as well as
the form of the resulting SOAC.

\Cref{sec:invalidfusion} goes into greater detail on invalid fusion,
and \cref{sec:whentofuse} covers cases in which fusion may actually be
...
\begin{bcolorcode}
let \{c\} = mapT(fn \{int\} (int x, int y) => \{x+y\}, a, b) in
reduceT(op +, 0, c)
    \emphh{\mymath{\Downarrow}}
reduceT(fn int (int acc, int x, int y) =>
          acc + x + y,
...
We can always fuse \texttt{map} with \texttt{redomap}.
...
\begin{align*}
  ...
  } \quad \mbox{($\overline{es_{a}}$ and $\overline{es_{b}}$ match)}
  \tagsc{Fuse-Map-Scan}
\end{align*}
...
This means that we can implement the reduction simply as a bottoms-up
...
\item[$\unfusable : Exp \rightarrow Names$.] The \textit{unfusable
    set}, which is key to preventing unwanted fusion, as it indicates
  which SOACs should never be fused.  The unfusable set prevents both
  undesired and invalid fusion, as outlined in sections
  \cref{sec:whentofuse,sec:invalidfusion} respectively.  Given an
  \LO{} expression $e$, we shall say $\unfusable(e)$ to refer to the
  unfusable set produced by $e$.  For example:
  \begin{align*}
